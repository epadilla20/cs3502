...
void* teller_thread(void* arg) {
	...
	// Small delay to simulate time between transactions
        usleep(100000);
    }
...
int main() {
    ...
    // Initialize account and its mutex
    ...
    // Destroy mutex after all threads finish
    pthread_mutex_destroy(&accounts[0].lock);

    printf("\nFinal balance: %.2f (Now consistent and thread-safe)\n", accounts[0].balance);
